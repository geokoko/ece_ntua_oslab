...
static int __init init_ext2_fs(void)
{
	...
	if (err) {
		printk(KERN_INFO "ext2-lite: cannot register filesystem\n");
...
static void __exit exit_ext2_fs(void)
{
    ...
    // Unregister the ext2-lite filesystem
    err = unregister_filesystem(&ext2_fs_type);
    if (err) {
        printk(KERN_ERR "ext2-lite: Failed to unregister filesystem (err: %d)\n", err);
        return;
    }

    printk(KERN_INFO "ext2-lite: Filesystem unregistered successfully\n");
